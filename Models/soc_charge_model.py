...
X = soc_charge_dataset.drop(columns=['soc']).values  # Features
y = soc_charge_dataset['soc'].values  # Target
...
# Save MinMaxScaling
joblib.dump(scaler, 'soc_charge_minmax_scaler.pkl')

# Convert scaled data back to DataFrame for consistency with original DataFrame 
X_train_scaled = pd.DataFrame(X_train_scaled, columns=soc_charge_dataset.drop(columns=['soc']).columns)
X_test_scaled = pd.DataFrame(X_test_scaled, columns=soc_charge_dataset.drop(columns=['soc']).columns)
...
model.save_weights('soc_charge_weights.h5')
...
plt.ylabel('Loss')
plt.legend()
plt.savefig("soc_charge_trainig.png")
...
plt.grid(True)
plt.legend()
plt.savefig("soc_charge_predictions.png")
